...
// guest call
static void do_load_bpf(fuse_req_t req, fuse_ino_t nodeid,
                         struct fuse_mbuf_iter *iter){
    if (req->se->debug){
        fprintf(stderr,
                "unique: %llu, opcode:  %u\n",
    ...
    if (req->se->op.loadbpf){
        // Before version 7.9, some fields were missing from the fuse_read_in struct
        bool compat = req->se->conn.proto_major < 9;
        struct fuse_read_in *arg;
        struct fuse_file_info fi;
        ...
        req->se->op.loadbpf(req, nodeid, arg->size, arg->offset, &fi);
    } else {
        fuse_reply_err(req, ENOSYS);
    }
}
...
static struct {
    ...
    [FUSE_LOAD_BPF] = {do_load_bpf, "LOAD BPF"},
};
