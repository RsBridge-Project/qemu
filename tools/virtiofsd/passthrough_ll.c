...
int load_bpf_program(char *path) {
    ...
    fuse_log(FUSE_LOG_DEBUG, "load_bpf_program(path=%s)\n", path);
    
    obj = bpf_object__open_file(path, NULL);
    if (!obj){
        fuse_log(FUSE_LOG_ERR,"Failed to open bpf program\n");
        return -1;
    }

    prog = bpf_object__find_program_by_name(obj, "oliver_agg_func");
    if (!prog) {
        fuse_log(FUSE_LOG_ERR,"Failed to find program\n");
        goto clean_up;
    }

    ret = bpf_program__set_type(prog, BPF_PROG_TYPE_XRP);
    if(ret){
        fuse_log(FUSE_LOG_ERR,"Failed to set program type\n");
        goto clean_up;
    }

    ret = bpf_object__load(obj);
    if(ret){
        fuse_log(FUSE_LOG_ERR,"Failed to load object\n");
        goto clean_up;
    }

    prog_fd = bpf_program__fd(prog);
    if (prog_fd <= 0){
        fuse_log(FUSE_LOG_ERR,"Failed to get program fd\n");
        goto clean_up;
    }
...
static int fuse_send_msg_bpf(struct fuse_session *se, struct fuse_chan *ch,
...
static void lo_load_bpf(fuse_req_t req, fuse_ino_t ino, size_t size,
                        off_t offset, struct fuse_file_info *fi) {
    const char *shared_path = SHARED_PATH;
    struct fuse_session *se = req->se;
    struct fuse_chan *ch = req->ch;
    int ret = 0;

    fuse_log(FUSE_LOG_DEBUG,
             "lo_load_bpf(ino=%" PRIu64 ", size=%zd, "
                                    "off=%lu, bpf_ino=%ld)\n",
            ino, size, (unsigned long)offset, req->bpf_ino);

    if (req->bpf_ino != 0 && !se->bpf_func) {
        char *bpf_path = find_path_by_ino(shared_path, req->bpf_ino);

        fuse_log(FUSE_LOG_DEBUG,"lo_load_bpf(bpf path: %s)\n", bpf_path);
    ...
    if (se->bpf_fd <= 0) {
        fuse_log(FUSE_LOG_DEBUG, "lo_load_bpf(bpf_fd=%d)\n", se->bpf_fd);
        ...
        iov[1].iov_base = &se->bpf_fd;
        iov[1].iov_len = sizeof(se->bpf_fd);

        fuse_send_msg_bpf(se, ch, iov, 2);
        fuse_free_req(req);
    }
